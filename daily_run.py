...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--current-rebound-max", type=float, default=0.05, help="当前低点到最新收盘涨幅上限，默认 0.05，对应 f20 策略")
    parser.add_argument("--full-rebound-min", type=float, default=0.20, help="历史完整反弹涨幅下限，默认 0.20，对应 f20 策略")
...
def main() -> None:
    ...
    rebound_args = [
        "--current-rebound-max",
        str(args.current_rebound_max),
        "--full-rebound-min",
        str(args.full_rebound_min),
    ]
    run_command([python, "filter_drawdown_rebound_history.py", "--progress", "--charts", str(args.charts), *rebound_args])
    run_command([python, "rank_rebound_candidates.py", *rebound_args])
