...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
用法:
  ./strategy_recommended_runner/run_livermore_50_breakout.sh [YYYY-MM-DD] [--skip-update] [--rebuild-events]
  ./strategy_recommended_runner/run_livermore_50_breakout.sh --date YYYY-MM-DD [--skip-update] [--rebuild-events]

默认流程:
  1. 更新 A 股日线 parquet
  2. 快速扫描指定信号日的 50 关键点首次突破候选
...
echo ""
echo "=== 导出 ${SIGNAL_DATE} 的 50 关键点首次突破候选 ==="
"${PYTHON_BIN}" strategy_recommended_runner/scripts/export_key_level_50_breakout_candidate_fast.py \
  --date "${SIGNAL_DATE}"
